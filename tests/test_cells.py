...
#   09:30 ok 700/500
# Corridor b has no tier: one call at 08:10.
SAMPLES = parsed([
...
CORRIDORS = pd.DataFrame({"corridor_id": ["a", "b"], "tier": ["A", None]})
...
def test_corridor_without_a_tier_falls_back_to_attempted_calls():
...
def test_schedule_on_local_hours_invents_no_slot_before_first_call():
    ...
    samples = parsed([("c", "2026-09-01 00:00", 200, 600, 500),
                      ("c", "2026-09-01 00:30", 200, 600, 500),
                      ("c", "2026-09-01 01:00", 200, 600, 500)])
    corridors = pd.DataFrame({"corridor_id": ["c"], "tier": ["B"]})
    cells = hourly_cells(samples, corridors)
    assert cells[["hour", "n_expected", "n_ok"]].values.tolist() == [[0, 2, 2], [1, 1, 1]]
    assert (cells["day"] == DAY).all()
    assert (cells["missing_rate"] == 0).all()


def test_hours_between_collection_windows_owe_no_calls():
    # Tier A: a call at 10:15, the last morning slot, then 16:45. Nothing is owed
    # from 10:30 to 16:30; the 16:30 slot has no call and is missing.
    samples = parsed([("d", "2026-09-01 10:15", 200, 600, 500),
                      ("d", "2026-09-01 16:45", 200, 700, 500)])
    cells = hourly_cells(samples, pd.DataFrame({"corridor_id": ["d"], "tier": ["A"]}))
    assert cells[["hour", "n_expected", "n_ok"]].values.tolist() == [[10, 1, 1], [16, 2, 1]]
...
def test_hour_with_no_success_has_nan_metrics_not_estimates():
    ...
    corridors = pd.DataFrame({"corridor_id": ["a"], "tier": [None]})
    c = cell(hourly_cells(samples, corridors), "a", 8)
    assert (c.n_expected, c.n_ok, c.missing_rate) == (2, 0, 1.0)
    assert np.isnan(c.tt_mean_s)
    assert "tt_p95_s" not in c.index  # a cell never carries a tail statistic
