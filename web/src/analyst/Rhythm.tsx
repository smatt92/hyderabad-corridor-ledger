...
import type { Corridor, HeatmapResponse } from "../api/types";
...
import { fmtCoverage, fmtHour, fmtMinutes, fmtNum, parseDay } from "../lib/format";
import { EM_DASH } from "../lib/route";
import { cache } from "./cache";
import { BASIS_LABEL, type Basis, Notice, SectionHead, Select, StatList, kicker } from "./common";
...
export function Rhythm({ corridors, day, hour }: { corridors: Corridor[]; day: string; hour: number }) {
  ...
  const head = (
    <SectionHead
      title="Weekly rhythm"
      sub={`Median TTI by hour and weekday, pooled over the ${"90"}-day window, against ${BASIS_LABEL[basis]}. The hour you leave matters more than the route you pick.`}
      right={controls}
    />
  );
  ...
  if (!res.ok) {
    return (
      <div>
        {head}
        <Notice kicker="Degraded" title="Rhythm matrix unavailable">The read API returned: {res.error}. Nothing is drawn rather than a guess.</Notice>
      </div>
    );
  }

  const matrix = basis === "tomtom" ? res.data.tti_tomtom_p50 : res.data.tti_p5_p50;
  const range = extent(matrix);
  const { worst, best } = extremes(matrix);
  const here = matrix[dow]?.[hour] ?? null;
  const readout =
    worst && best
      ? `On ${corridor.name} the worst published hour of the week is ${DAYS[worst.dow]} ${fmtHour(worst.hour)} (median TTI ${fmtNum(worst.v)}); the lightest is ${DAYS[best.dow]} ${fmtHour(best.hour)} (${fmtNum(best.v)}). Dashed cells are hour-weekday pairs with too few days to publish.`
      : "No hour-weekday cell on this corridor has enough days to publish yet. Dashed cells are left empty, not estimated.";
    ...
    <div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "minmax(0,1.5fr) minmax(280px,1fr)", gap: "28px", alignItems: "start" }}>
        <div style={{ minWidth: 0 }}>
          <RhythmMatrix matrix={matrix} style={ANALYST_RHYTHM} highlight={{ dow, hour }} />
        </div>
        <div style={{ display: "flex", flexDirection: "column", gap: "16px", maxWidth: "46ch" }}>
          <div>
            ...
            <div style={{ marginTop: "6px", fontSize: "11px", color: MID, lineHeight: 1.5 }}>
              Lightness is monotonic, so the scale stays ordered under deuteranopia and in greyscale print. Colour encodes magnitude only.
            </div>
          </div>
          ...
          <div style={{ borderTop: `1px solid ${FAINT}`, paddingTop: "14px" }}>
            <StatList
              items={[
                ...
                [`median TTI ${DAYS[dow]!.slice(0, 3)} ${fmtHour(hour)}`, here == null ? `${EM_DASH} (not published)` : fmtNum(here)],
